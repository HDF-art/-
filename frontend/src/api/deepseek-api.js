...
class DeepSeekAPI {
  ...
  _buildAnalysisPrompt(identifyResult) {
    ...
    return `你是一位专业的农业病虫害专家，请对以下病虫害识别结果进行详细分析：
...
1. 基本信息：病虫害类型、主要危害作物、发病高峰期、传播途径
2. 症状描述：典型症状特征、发病部位、发展阶段特征、与相似病虫害的区别
3. 危害程度：对作物生长的影响、产量损失预估、经济影响
4. 防治措施：农业防治方法、化学防治方法及推荐药剂、生物防治方法、物理防治方法
5. 预防建议：种植前预防措施、生长期预防措施、收获后预防措施、轮作倒茬建议
6. 用药指南：推荐药剂及用量、用药时间和频次、安全间隔期、注意事项
7. 识别评价：对当前识别结果的评价、提高识别准确率的建议、进一步确认的方法

请确保返回的JSON格式正确，所有字段都包含专业、准确且实用的中文内容。`
  }
